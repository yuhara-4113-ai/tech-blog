...
ちょうどChatGPT Plusを1か月無料で試せたので、使ってみることにしました。

CodexはWebアプリ開発だけでなく、リポジトリを使った家族旅行の計画や職歴の整理にも利用しています。
思った以上に用途が広く、無料期間が終わってもそのまま契約を続けちゃいそうです。。。

このアプリもCodexを使って開発し、実装の指示はほぼスマートフォンから日本語で出しました。

この記事もCodexに執筆を依頼しています。
伝えたい内容や文章の違和感を私が伝え、生成された文章を確認しながら修正しました。
...
入力内容はリアルタイムでカードへ反映され、完成したカードは高解像度PNGで保存できます。
...
完成には至らず、GitHubに残っているのはREADMEとライセンスだけです。
...
私自身の依頼方法も1年前とは変わっているため、CopilotとCodexを単純には比較できません。
...
スマホ側のCodexは、接続先PCにあるプロジェクトやファイルを利用します。
対象リポジトリを事前に登録し、PC側のCodexアプリを起動したままにしておく必要があります。
PCがスリープすると接続は止まります。
...
この流れを作ったことで、PCの前にいないときでも開発を進められました。
...
そのため、プレビュー用と保存用に別々のカードを実装せずに済みます。
...
光沢を有効にして保存する場合は、Canvas上で光沢表現を合成します。
...
家族の写真を使うため、画像をサーバーへ送らないことは最初から意識していました。
...
残念ながら、今回使った一般ユーザー向けのGemini Code Assist on GitHubは2026年7月17日に終了しました。
Enterprise版は終了の対象外です。
...
複数の機能を一度に依頼すると確認が難しくなるため、1つの機能ごとに実装、lint、build、ブラウザ確認を行いました。
...
寝ながらでも出先でもスマホから指示でき、ここまでPCを開かずに開発できるとは思っていませんでした。
